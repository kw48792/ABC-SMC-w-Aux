...
ABC_SMC <- function(Time, g, train_index, res.old, w.old, Sigma, eps,i){
    ...
    if(prior.non.zero(rbind(r_g,k_s,k_c,r_d,sd_rho,sd_I))){
      ...
      for (j in 1:n) {
        for(r in train_index){
          ...
          # r_g_aug <- c(rep(r_g[1],6),rep(r_g[2],4))
          D_star<-run_model_t(r_g_aug, k_s_aug, k_c_aug, r_d_aug, sd_rho_aug, sd_I_aug, r, n_time)
          # Calculate distances
  ...
  return(c(res,w1/w2,sum(distance)))
}
...
write.table(res.old,paste0("results_para",mrep,".txt"),sep=",",append=TRUE,eol="\n",
            row.names=FALSE,col.names=FALSE);
write.table(w.old,paste0("results_weight",mrep,".txt"),sep=",",append=TRUE,eol="\n",
            row.names=FALSE,col.names=FALSE);
write.table(dis.old,paste0("results_distance",mrep,".txt"),sep=",",append=TRUE,eol="\n",
            row.names=FALSE,col.names=FALSE);
write.table(time_cost,paste0("results_time",mrep,".txt"),sep=",",append=TRUE,eol="\n",
            row.names=FALSE,col.names=FALSE);
